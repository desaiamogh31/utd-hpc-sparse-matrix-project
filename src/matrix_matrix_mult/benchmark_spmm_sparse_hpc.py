"""
Sparse B SpMM Benchmarking Suite (OpenMP HPC)

Benchmarks sparse matrix-matrix multiplication C = A @ B on a single HPC node:
  - A: Sparse matrix loaded from local cache / Phase 2 cache / SuiteSparse fallback
  - B: Randomly generated sparse matrix
  - Three custom OpenMP algorithms + SciPy built-in
  - Includes the larger matrices that are skipped by the laptop-oriented benchmark

Usage:
    python benchmark_spmm_sparse_hpc.py

Sample usage:
    python benchmark_spmm_sparse_hpc.py --outdir results_hpc_spmm --threads 1 2 4 8 16 32 64
"""
...
def benchmark_matrix_sparse_b_hpc(
    A,
    matrix_name: str,
    b_cols: list[int],
    ...
    """Benchmark one matrix across sparse-B choices and OpenMP thread counts."""
...
def benchmark_all_matrices_sparse_b_hpc(
    cache_dir: str,
    ...
    """Run the HPC sparse-B SpMM benchmark over the requested matrix set."""
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, output_csv)

    print("=" * 90)
    print("Sparse B SpMM OpenMP HPC Benchmark")
    print("=" * 90)
...
def plot_results(df: pd.DataFrame, output_dir: str) -> None:
    """Generate the same analysis plots as the OpenMP local benchmark, sized for HPC runs."""
    os.makedirs(output_dir, exist_ok=True)
    ...
    fig.suptitle(
        f"Sparse B SpMM OpenMP HPC: Algorithm Comparison\nB sparsity: {sparsity_label}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "spmm_sparse_hpc_algorithm_comparison.png")
    ...
    fig.suptitle(
        f"Sparse B SpMM OpenMP HPC: B Column Count and Performance\nB sparsity: {sparsity_label}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "spmm_sparse_hpc_scaling.png")
    plt.savefig(plot_path, dpi=150, bbox_inches="tight")
    print(f"  ✓ Saved: {plot_path}")
    plt.close()
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "spmm_sparse_hpc_thread_scaling.png")
    plt.savefig(plot_path, dpi=150, bbox_inches="tight")
    print(f"  ✓ Saved: {plot_path}")
    plt.close()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark sparse-B SpMM with OpenMP on HPC, including larger matrices."
    )
    parser.add_argument(
        "--output",
        "-o",
        default="benchmark_spmm_sparse_hpc.csv",
        help="Output CSV filename (default: benchmark_spmm_sparse_hpc.csv)",
    )
    parser.add_argument(
        "--outdir",
        default="results_hpc_spmm",
        help="Output directory (default: results_hpc_spmm)",
    )
    parser.add_argument(
        "--cache-dir",
        default="matrices/",
    ...
    parser.add_argument(
        ...
        help="OpenMP thread counts to benchmark (default: 1 2 4 8 16 32 64)",
    )
    parser.add_argument(
        "--matrices",
        nargs="+",
        ...
        help=(
            "Matrix names to benchmark "
            f"(default: {' '.join(HPC_DEFAULT_MATRICES)})"
        ),
    )
    args = parser.parse_args()

    benchmark_all_matrices_sparse_b_hpc(
